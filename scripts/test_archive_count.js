...
async function main() {
  ...
  assert.equal(combined.elements["archive-count"].textContent, "1,202");
  assert.match(combined.elements["archive-count-status"].textContent, /1,200 release assets \+ 2 large files/);
  ...
  assert.equal(live.interval.delay, 5 * 60 * 1000);
